        ...
        while True:
            try:
                ...
                if data[0] == 0xF7 and data[-1] == 0xFA:
                    ...
                    # 큐에 데이터 저장
                    airflow_data_queue.append(data.hex())
                    logger.info("패킷 데이터가 큐에 저장되었습니다.")
                    await websocket.send_text("AIRFLOW data received successfully.")
                else:
...
async def get_airflow():
    ...
    if not airflow_data_queue:  # 데이터가 비어있는 경우
        return {"message": "No AIRFLOW data available.", "data": []}
    return {"message": "AIRFLOW 데이터 조회 성공", "data": list(airflow_data_queue)}
