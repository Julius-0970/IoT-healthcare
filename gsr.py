...
# GSR 데이터를 파싱하는 함수
def parse_gsr_data(raw_data_hex):
    """
    수신된 원시 GSR 데이터(hex 문자열)를 파싱하여 실제 값 리스트로 변환합니다.
    패킷 구조:
    - SOP (1바이트): f7
    - CMD (1바이트): 82
    ...
    try:
        while True:
            ...
            try:
                ...
                # 데이터를 파싱하여 큐에 저장
                parsed_values = parse_gsr_data(raw_data_hex)
                if parsed_values:
...
# GSR 데이터를 조회하는 기존 HTTP GET 엔드포인트
@gsr_router.get("/gsr")
async def get_gsr():
    """
    큐에 저장된 파싱된 GSR 데이터를 반환하는 HTTP GET 엔드포인트.
    """
    if not gsr_data_queue:
        return {
            "status": "error",
            "message": "No GSR data available.",
            "data": []
        }
    return {
        "status": "success",
        "message": "GSR 데이터 조회 성공",
        "data": list(gsr_data_queue)
    }
